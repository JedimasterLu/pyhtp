...
class XrdProcess:
    ...
    def combine_data(self, left_data: tuple[np.ndarray, np.ndarray], right_data: tuple[np.ndarray, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
        ...
        left_data_avg = left_data[(left_data['two_theta'] > 33.5) & (left_data['two_theta'] < 34)]
        right_data_avg = right_data[(right_data['two_theta'] > 34) & (right_data['two_theta'] < 34.5)]
        left_avg = left_data_avg['intensity'].mean()
        right_avg = right_data_avg['intensity'].mean()
        # Normalize the intensity of the two data
        left_data['intensity'] = left_data['intensity'] + right_avg - left_avg
