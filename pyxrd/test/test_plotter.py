...
from pyxrd.plotter import XrdPlotter, _get_width
...
class TestXrdPlotter(unittest.TestCase):
    ...
    def test_init(self):
        ...
        plotter = XrdPlotter(
            file_dir=['pyxrd/test/test_xy/22-analyze/',
                      'pyxrd/test/test_xy/46-analyze/']
        )
        ...
        with self.assertRaises(ValueError):
            XrdPlotter(file_dir=['pyxrd/test/test_xy/22-analyze/'])
    ...
    def test_plot_spectrum_with_ref(self):
        ...
        ax = plotter.plot_spectrum_with_ref(
            ...
            if_save=False,
            if_show=False,
            database_dir='pyxrd/test/test_cif/',
            plot_type='stack',
            save_path='pyxrd/test/test_xy/test_fig.png'
        )
        self.assertIsInstance(ax, mpl.axes.Axes)
        ...
        ax[2] = plotter.plot_spectrum_with_ref(ax=ax[2], index_to_plot=[0, 1], plot_type='stack', if_show=False, database_dir='pyxrd/test/test_cif/')
    ...
    def test_set_save_dir(self):
        ...
        plotter.set_save_dir('pyxrd/test/test_xy/')
        self.assertIsNotNone(plotter.save_dir)
