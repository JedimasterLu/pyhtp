...
class TestXrdProcess(unittest.TestCase):
    ...
    def test_init(self):
        ...
        model = XrdProcess(
            file_path=['pyxrd/test/test_xy/22-analyze/22-000-0000_exported.xy',
                       'pyxrd/test/test_xy/46-analyze/46-000-0000_exported.xy'],
        )
    ...
    def test_copy(self):
        ...
        model = XrdProcess(
            file_path=['pyxrd/test/test_xy/22-analyze/22-000-0000_exported.xy',
                       'pyxrd/test/test_xy/46-analyze/46-000-0000_exported.xy'],
            pattern_path='pyxrd/test/test_cif/pattern.pkl',
            structure_path='pyxrd/test/test_cif/structure.pkl',
        )
    ...
    def test_set_database_path(self):
        ...
        self.assertEqual(model.pattern_path, 'pyxrd/test/test_cif/pattern.pkl')
        self.assertEqual(model.structure_path, 'pyxrd/test/test_cif/structure.pkl')
    ...
    def test_peaks(self):
        ...
        with self.assertRaises(ValueError):
            peaks_value, peaks_index, properties = model.peaks(mask=[[33, 25, 27]])
        peaks_value, _, _ = model.substract_baseline().smooth().peaks(mask=[[33, 35.5]], mask_height=0.1, height=0.06)
        self.assertEqual(len(peaks_value), 3)
        self.assertEqual(len(peaks_index), 3)
        with self.assertRaises(ValueError):
            model = XrdProcess()
    ...
    def test_match(self):
        ...
        with self.assertRaises(ValueError):
            ...
            model = model.match()
        model = XrdProcess(
            file_path=['pyxrd/test/test_xy/22-analyze/22-208-0000_exported.xy',
                       'pyxrd/test/test_xy/46-analyze/46-208-0000_exported.xy'],
        )
        model = model.substract_baseline().smooth()
        model.set_database_path(
            pattern_path='pyxrd/test/test_cif/pattern.pkl',
